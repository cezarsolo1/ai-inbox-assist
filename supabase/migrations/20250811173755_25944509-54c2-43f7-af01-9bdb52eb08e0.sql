-- Email conversations schema (retry without IF NOT EXISTS for policies)
...
create policy "Allow anon to select email_threads"
  on public.email_threads for select using (true);

create policy "Allow anon to select email_messages"
...
create policy "Service role can update threads"
  on public.email_threads for update using (true) with check (true);

create policy "Service role can delete threads"
...
create policy "Allow anon to update seen email_messages"
  on public.email_messages for update using (true) with check (true);
