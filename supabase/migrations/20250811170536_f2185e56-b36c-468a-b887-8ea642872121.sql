-- Restrict anon updates to only the 'seen' column via trigger; allow service_role full updates

-- Create a function to validate updates
...
DROP TRIGGER IF EXISTS trg_inbox_messages_validate_update ON public.inbox_messages;
CREATE TRIGGER trg_inbox_messages_validate_update
BEFORE UPDATE ON public.inbox_messages
FOR EACH ROW
EXECUTE FUNCTION public.inbox_messages_validate_update();

-- Add a policy to allow service_role to update any row
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'inbox_messages' AND policyname = 'Service role can update messages'
  ) THEN
